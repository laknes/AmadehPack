...
در مرحله تنظیم دیتابیس، وارد کردن `DATABASE_URL` معتبر الزامی است و باید با `postgresql://` یا `postgres://` شروع شود. در حالت unattended نیز مقدار `DATABASE_URL` باید از قبل در محیط تعریف شده باشد؛ در غیر این صورت نصب متوقف می‌شود.
