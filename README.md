...
در مرحله تنظیم دیتابیس، اگر `DATABASE_URL` تعریف نشده باشد، اسکریپت به‌صورت خودکار از PostgreSQL محلی با مقدار `postgresql://postgres:postgres@127.0.0.1:5432/amadeh_pack?schema=public` استفاده می‌کند. برای دیتابیس دیگر، مقدار `DATABASE_URL` را قبل از اجرا تنظیم کنید. SSL نیز اختیاری است و به‌صورت پیش‌فرض فعال نمی‌شود.
