...
export function SiteFooter() {
  return (
    <footer className="mt-20 border-t border-[#B8874B]/25 bg-[#FFF4DF]">
      <div className="container grid gap-8 py-12 lg:grid-cols-[1.4fr_1fr_1fr_1fr]">
        <section className="glass rounded-[28px] p-6">
          <div className="mb-5 flex items-center gap-3">
            <span className="grid size-12 place-items-center rounded-2xl bg-[#8B5A2B] text-[#FFF4DF] font-black">KP</span>
            <div>
              <strong className="text-xl">کرافت پک</strong>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            {[["ارسال عمده", Truck], ["پرداخت امن", ShieldCheck], ["چاپ اختصاصی", Sparkles]].map(([label, Icon]) => (
              <div key={String(label)} className="rounded-2xl border border-[#B8874B]/25 bg-white p-3 text-sm text-[#4A2F1B]">
                <Icon className="mb-2 text-[#8B5A2B]" size={20} />
                {label as string}
              </div>
            ))}
          </div>
      ...
      <div className="border-t border-[#B8874B]/25 py-5 text-center text-sm text-[#6F6256]">© 2026 Kraft Pack. طراحی شده برای فروش B2B و B2C.</div>
    </footer>
  );
}
